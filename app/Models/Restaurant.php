...
class Restaurant extends Model
{
    ...
    protected $fillable = ['food_section','status','new_available_times'];
    ...
    public function scopeWithOpen($query,$longitude,$latitude, $mainCategoryId = null)
    {
        $currentTime = now()->format('H:i:s');
        $currentDayIso = (int) now()->format('N'); // 1 (Monday) - 7 (Sunday)

        // If main_category_id not provided, detect from current time
        if (!$mainCategoryId) {
        ...
        // Build schedule query - check if restaurant has schedule for current meal category
        $dayConditions = '`restaurant_schedule`.`day` = '.$currentDayIso;

        $scheduleQuery = '(select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id` and ('.$dayConditions.') and `restaurant_schedule`.`opening_time` <= "'.$currentTime.'" and `restaurant_schedule`.`closing_time` >= "'.$currentTime.'"';

        // Add main_category_id check - if specific category, check only that; otherwise check any
        if ($mainCategoryId) {
        ...
        $query->selectRaw('*, IF('.$scheduleQuery.', true, false) as open,ST_Distance_Sphere(point(longitude, latitude),point('.$longitude.', '.$latitude.')) as distance');
    }
