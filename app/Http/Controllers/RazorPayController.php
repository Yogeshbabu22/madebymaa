    ...
    public function __construct(PaymentRequest $payment, User $user)
{
    ...
    if ($razor) {
        ...
        // Set for global use
        Config::set('razor_config', $this->config);
    } else {
    ...
    public function payment(Request $request): JsonResponse|Redirector|RedirectResponse|Application
{
    try {
        ...
        if ($validator->fails()) {
            \Log::error('Razorpay validation failed:', $validator->errors()->toArray());
        ...
        if (empty(config('razor_config.api_key')) || empty(config('razor_config.api_secret'))) {
