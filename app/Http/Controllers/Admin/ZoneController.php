...
class ZoneController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|unique:zones|max:191',
            'coordinates' => 'required',
            'radius' => 'nullable|numeric|min:0', // Add validation for radius
        ]);
        ...
        $zone->maximum_shipping_charge = $request->maximum_shipping_charge ?? null;
        $zone->max_cod_order_amount = $request->max_cod_order_amount ?? null;
        $zone->radius = $request->radius ?? null; // Save the radius value
        $zone->save();
       ...
       public function update(Request $request, $id)
{
    $request->validate([
        'name' => 'required|max:191|unique:zones,name,' . $id,
        'coordinates' => 'required',
        'radius' => 'required|numeric|min:1', // Validation for radius
    ]);
    ...
    $zone->customer_wise_topic = 'zone_' . $id . '_customer';
    $zone->deliveryman_wise_topic = 'zone_' . $id . '_delivery_man';
    $zone->radius = $request->radius; // Assign the radius value
    $zone->save();
    ...
    if ($request->ajax()) {
        return response()->json([
            'redirect_url' => route('admin.zone.home'),
            'message' => translate('messages.zone_updated_successfully'),
        ]);
    }
    return redirect()->route('admin.zone.home');
}
