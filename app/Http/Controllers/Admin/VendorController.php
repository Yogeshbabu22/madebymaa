...
class VendorController extends Controller
{
    public function index()
    {
        ...
        return view('admin-views.vendor.index', compact('page_data', 'mainCategories')) ;
    }
    ...
    public function store(Request $request)
    {
        ...
        // Save availability schedule with main categories
        if ($request->has('availability_data') && !empty($request->availability_data)) {
            try {
                ...
                \Illuminate\Support\Facades\Log::info('Availability data received: ' . $request->availability_data);
                if (json_last_error() === JSON_ERROR_NONE && is_array($availabilityData)) {
                    ...
                    foreach ($availabilityData as $day => $categoriesData) {
                        ...
                        foreach ($categoriesData as $categoryName => $categoryInfo) {
                            ...
                            try {
                                ...
                                \Illuminate\Support\Facades\Log::info('RestaurantSchedule created successfully: ' . json_encode($schedule->toArray()));
                            } catch (\Exception $e) {
                                // Log error but continue processing other days
                                \Illuminate\Support\Facades\Log::error('RestaurantSchedule creation error: ' . $e->getMessage());
                                \Illuminate\Support\Facades\Log::error('RestaurantSchedule creation error trace: ' . $e->getTraceAsString());
                            }
                        }
                    }
                    ...
                    $restaurant->save();
                    \Illuminate\Support\Facades\Log::info('Restaurant new_available_times saved successfully');
                } else {
                    \Illuminate\Support\Facades\Log::error('Invalid availability data format. JSON error: ' . json_last_error_msg());
                }
            } catch (\Exception $e) {
                \Illuminate\Support\Facades\Log::error('Availability data processing error: ' . $e->getMessage());
                \Illuminate\Support\Facades\Log::error('Availability data processing error trace: ' . $e->getTraceAsString());
            }
        } else {
            \Illuminate\Support\Facades\Log::info('No availability_data received in request');
        }
    ...
    public function edit($id)
    {
        ...
        return view('admin-views.vendor.edit', compact('restaurant', 'mainCategories', 'existingAvailability'));
    }
    ...
    public function update(Request $request, Restaurant $restaurant)
    {
        ...
        if ($request->filled('password')) {
            ...
            if ($passwordValidator->fails()) {
                $validator->getMessageBag()->merge($passwordValidator->errors());
            }
        }
        ...
        if ($request->filled('password') && strlen($request->password) > 0) {
            $vendor->password = bcrypt($request->password);
        } else {
        ...
        if ($request->has('availability_data') && !empty($request->availability_data)) {
            try {
                $availabilityData = json_decode($request->availability_data, true);
                \Illuminate\Support\Facades\Log::info('Update - Availability data received: ' . $request->availability_data);
                if (json_last_error() === JSON_ERROR_NONE && is_array($availabilityData)) {
                    ...
                    foreach ($availabilityData as $day => $categoriesData) {
                        ...
                        foreach ($categoriesData as $categoryName => $categoryInfo) {
                            ...
                            try {
                                ...
                                \Illuminate\Support\Facades\Log::info('Update - RestaurantSchedule created successfully: ' . json_encode($schedule->toArray()));
                            } catch (\Exception $e) {
                                // Log error but continue processing other days
                                \Illuminate\Support\Facades\Log::error('Update - RestaurantSchedule creation error: ' . $e->getMessage());
                                \Illuminate\Support\Facades\Log::error('Update - RestaurantSchedule creation error trace: ' . $e->getTraceAsString());
                            }
                        }
                    }
                    ...
                    $restaurant->new_available_times = json_encode($availabilityData);
                    $restaurant->save();
                    \Illuminate\Support\Facades\Log::info('Update - Restaurant new_available_times saved successfully');
                } else {
                    \Illuminate\Support\Facades\Log::error('Update - Invalid availability data format. JSON error: ' . json_last_error_msg());
                }
            } catch (\Exception $e) {
                \Illuminate\Support\Facades\Log::error('Update - Availability data processing error: ' . $e->getMessage());
                \Illuminate\Support\Facades\Log::error('Update - Availability data processing error trace: ' . $e->getTraceAsString());
            }
        } else {
            \Illuminate\Support\Facades\Log::info('Update - No availability_data received in request');
        }
    ...
    public function pending(Request $request)
    {
        ...
        $restaurants = Restaurant::when(is_numeric($zone_id), function ($query) use ($zone_id) {
            ...
            ->type($type)->RestaurantModel($typ)->latest()->paginate(config('default_pagination'));
                $zone = is_numeric($zone_id) ? Zone::findOrFail($zone_id) : null;
        $restaurant_penings = RestaurantPendingList::when(isset($key),function($query)use($key){
                $query->where(function($q)use($key){
                    foreach ($key as $value) {
                        $q->orWhere('f_name', 'like', "%{$value}%")
                        ->orWhere('email', 'like', "%{$value}%")
                        ->orWhere('mobile_no', 'like', "%{$value}%");
                            ...
                            ->where('status', '!=', 'approved')->where('status', '!=', 'rejected')->orderBy('id','desc')->get();

        $search_value = $request['search'] ?? '';
        return view('admin-views.vendor.pending_list', compact('restaurants', 'zone', 'type','typ','restaurant_penings','key','search_value'));
    }
     public function approved(Request $request)
    {
        ...
        $typ = $request->query('restaurant_model', '');
        $search_value = $request['search'] ?? '';
        $restaurants = RestaurantPendingList::when(isset($key),function($query)use($key){
                    $query->where(function($q)use($key){
                        foreach ($key as $value) {
                            $q->orWhere('f_name', 'like', "%{$value}%")
                            ->orWhere('email', 'like', "%{$value}%")
                ...
                })->where("status","=","approved")->where("confirm_status",1)->where("register_status",0)->get();
                $zone = is_numeric($zone_id) ? Zone::findOrFail($zone_id) : null;

        return view('admin-views.vendor.approved', compact('restaurants', 'zone', 'type','typ','search_value'));
    }
    public function denied(Request $request)
    {
        ...
        $restaurants = RestaurantPendingList::when(isset($key),function($query)use($key){
                ...
                })->where("status","=","rejected")->where("confirm_status",0)->get();
                $zone = is_numeric($zone_id) ? Zone::findOrFail($zone_id) : null;
        return view('admin-views.vendor.denied', compact('restaurants', 'zone', 'type','typ','search_value'));
    }
...
//        Helpers::gen_mpdf(view: $mpdf_view,file_prefix: 'qr-code' . rand(00001, 99999),file_postfix: $restaurant->id);
    ...
    public function status_change($id,$status){
        ...
        if($id == "" || $status == ""){
        Toastr::danger(translate('Id and Status is Required'));
         return back();
        }

        if($id != "" && $status == "rejected"){
            RestaurantPendingList::where('id',$id)->update([
                'status'=>'rejected',
                'confirm_status'=>0
            ]);
