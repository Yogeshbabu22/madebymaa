...
class BusinessSettingsController extends Controller
{
    ...
    // {
    //     // Only validate if it's not just third party delivery settings update
    //     // Check if third party delivery fields are present (means it's third party form)
    //     $isThirdPartyForm = $request->has('adloggs_api_key') || $request->has('third_party_delivery_status');

    //     // Only validate order settings if it's not third party form
    //     if (!$isThirdPartyForm) {
    ...
    //     // Only update order settings if it's NOT a third party form
    //     // If it's third party form, preserve existing order settings
    //     if (!$isThirdPartyForm) {
    ...
    //     $thirdPartyDeliveryStatus = $request->has('third_party_delivery_status') ? 1 : 0;
    //     BusinessSetting::updateOrInsert(['key' => 'third_party_delivery_status'], [
    //         'value' => $thirdPartyDeliveryStatus
    //     ]);

    //     // Third Party Delivery Config
    //     $thirdPartyDeliveryConfig = [
    //         'default_country' => $request['default_country'] ?? 'India',
    //         'order_category' => $request['order_category'] ?? 'Food and Beverage',
    //         'default_weight_kg' => $request['default_weight_kg'] ?? '1.00',
    //         'utc_offset' => (int)($request['utc_offset'] ?? 330)
    //     ];
    //     BusinessSetting::updateOrInsert(['key' => 'third_party_delivery_config'], [
    //         'value' => json_encode($thirdPartyDeliveryConfig)
    //     ]);

    //     // Adloggs Config - Get existing config first to preserve values if not provided
    //     $existingAdloggsConfig = BusinessSetting::where('key', 'adloggs_config')->first();
    //     $existingConfig = $existingAdloggsConfig ? json_decode($existingAdloggsConfig->value, true) : [];

    //     $adloggsConfig = [
    //         'api_key' => $request['adloggs_api_key'] ?? $existingConfig['api_key'] ?? '',
    //         'base_url' => $request['adloggs_base_url'] ?? $existingConfig['base_url'] ?? 'https://dev.adloggs.com',
    //         'partner_merchant_id' => $request['adloggs_partner_merchant_id'] ?? $existingConfig['partner_merchant_id'] ?? '5421'
    //     ];
    //     BusinessSetting::updateOrInsert(['key' => 'adloggs_config'], [
    //         'value' => json_encode($adloggsConfig)
    //     ]);

    //     // Log for debugging
    //     \Illuminate\Support\Facades\Log::info('Third Party Delivery Settings Updated', [
    //         'third_party_delivery_status' => $thirdPartyDeliveryStatus,
    //         'adloggs_config' => $adloggsConfig
    //     ]);
...
public function update_order(Request $request)
{
    /*
    |--------------------------------------------------------------------------
    | 1. Validation for Order Settings (Only if order settings form submitted)
    |--------------------------------------------------------------------------
    */

    $isOrderSettingsForm = $request->has('home_delivery') 
        || $request->has('take_away') 
        || $request->has('instant_order') 
        || $request->has('schedule_order');

    if ($isOrderSettingsForm) {
        ...
        /*
        |--------------------------------------------------------------------------
        | 2. UPDATE ORDER SETTINGS ALWAYS
        |--------------------------------------------------------------------------
        */
        ...
        BusinessSetting::updateOrInsert(['key' => 'schedule_order'], [
            'value' => $request->schedule_order ?? 0
        ]);

        BusinessSetting::updateOrInsert(['key' => 'home_delivery'], [
            'value' => $request->home_delivery ?? 0
        ]);
        ...
        BusinessSetting::updateOrInsert(['key' => 'repeat_order_option'], [
            'value' => $request->repeat_order_option ?? 0
        ]);
        ...
        $slot = $request->schedule_order_slot_duration;
        if ($request->schedule_order_slot_duration_time_formate === 'hour') {
            $slot = $slot * 60;
        }
        ...
        BusinessSetting::updateOrInsert(['key' => 'canceled_by_restaurant'], [
            'value' => $request->canceled_by_restaurant ?? 0
        ]);

        BusinessSetting::updateOrInsert(['key' => 'canceled_by_deliveryman'], [
            'value' => $request->canceled_by_deliveryman ?? 0
        ]);

        BusinessSetting::updateOrInsert(['key' => 'order_confirmation_model'], [
            'value' => $request->order_confirmation_model ?? 'restaurant'
        ]);
    }

    /*
    |--------------------------------------------------------------------------
    | 3. THIRD PARTY DELIVERY SETTINGS (Always update)
    |--------------------------------------------------------------------------
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | 4. ADLOGGS CONFIG (Keep old values if new not provided)
    |--------------------------------------------------------------------------
    */

    $existingConfig = BusinessSetting::where('key', 'adloggs_config')->first();
    $existingConfig = $existingConfig ? json_decode($existingConfig->value, true) : [];
...

    {
        ...
        BusinessSetting::query()->updateOrInsert(['key' => 'business_name'], [
         ...
         BusinessSetting::query()->updateOrInsert(['key' => 'add_radius'], [
            'value' => $request['add_radius']
        ]);
