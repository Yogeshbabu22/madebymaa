...
class OrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        if ($request->order_status == 'delivered') {
            ...
            $orderAddlogg = OrderAddloggsDelivery::where('order_id',$order->id)->first();

            $order->cancellation_reason = $request->reason;
            $order->canceled_by = 'admin';
            if ($orderAddlogg && $orderAddlogg->addlogg_order_id) {
                $cancelData = [
                    'order_uuid' => $orderAddlogg->addlogg_order_id,
                    'order_cancel_description' => $request->reason
                ];

                $apiKey = "b7a7d1d8-d61f-41e8-9e2a-43542a3c5409";
                $apiUrl = "https://dev.adloggs.com/aa/oporder/v1.2/cancel";
                Http::withHeaders(['x-api-key' => $apiKey])->post($apiUrl, $cancelData);
            }
