        ...
        public function storeItem(Request $request)
{
    $validator = Validator::make($request->all(), [
        ...
        'title.0' => 'required',
        'description.0' => 'required',
        'ingredients' => 'nullable|max:1000', // Allow ingredients to be null or a string
    ], [
        'category_id.required' => translate('messages.select_category'),
        'title.0.required' => translate('default_title_is_required'),
        'description.0.required' => translate('default_description_is_required'),
        'ingredients.max' => translate('messages.ingredients_max_limit'), // Custom error message for ingredients
    ]);
    ...
    // Save the ingredients as a comma-separated string
    $campaign->ingredients = is_array($request->ingredients) 
  ...
  public function updateItem(ItemCampaign $campaign, Request $request)
{
    ...
    // Handle the discount calculation logic
    if ($request['discount_type'] == 'percent') {
        $dis = ($request['price'] / 100) * $request['discount'];
    } else {
        $dis = $request['discount'];
    ...
    // Handle choice options and variations
    $campaign->choice_options = json_encode([]);
    $variations = [];
    if ($request?->options) {
        foreach (array_values($request->options) as $key => $option) {
    ...
    // Handle the ingredients logic
    $ingredients = $request->ingredients;
    if (is_array($ingredients)) {
        // Join array elements into a string with commas if it's an array
        $ingredients = implode(', ', $ingredients);
    } elseif ($this->isJson($ingredients)) {
        // If it's a JSON string, decode it and join elements if it's an array
    ...
    // Force ingredients to be a string even if it's empty or null
    ...
    // Assign the processed ingredients string
    $campaign->ingredients = $ingredients;
