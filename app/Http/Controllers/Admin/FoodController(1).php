...
public function store(Request $request)
{
    try {
        ...
        // Initialize an empty array for availability times
        $availableTimes = [];

        // Loop through each day of the week and store time slots
        $days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
        foreach ($days as $day) {
            if ($request->has("{$day}_start") && $request->has("{$day}_end")) {
                $daySlots = [];
                foreach ($request->input("{$day}_start") as $key => $startTime) {
                    $endTime = $request->input("{$day}_end")[$key] ?? null;
        ...
        // Encode availability times as JSON
        $encodedAvailableTimes = json_encode($availableTimes);
        ...
        if ($request->has('super_category_ids') && is_array($request->super_category_ids)) {
            $food->super_category_ids = $request->super_category_ids;
        } else {
        ...
        // Save the new availability times
        $food->new_available_times = $encodedAvailableTimes;
    ...
    public function edit($id)
    {
        ...
        // Derive category selections reliably from category_ids (handles objects/arrays and order)
        $rootCategoryId = null;
        $subCategoryId = null;
        $subSubCategoryId = null;
        if ($product_category) {
            foreach ($product_category as $entry) {
                $position = is_array($entry) ? ($entry['position'] ?? null) : ($entry->position ?? null);
                $cid = is_array($entry) ? ($entry['id'] ?? null) : ($entry->id ?? null);
                if ($position == 1 && $cid !== null) { $rootCategoryId = (int) $cid; }
                if ($position == 2 && $cid !== null) { $subCategoryId = (int) $cid; }
                if ($position == 3 && $cid !== null) { $subSubCategoryId = (int) $cid; }
            }
        }
        // Normalize super_category_ids to an array for the edit form (handles JSON, CSV strings)
        $productSuperCategoryIds = $product->super_category_ids;
        if (is_string($productSuperCategoryIds)) {
            $decoded = json_decode($productSuperCategoryIds, true);
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                $productSuperCategoryIds = $decoded;
            } else {
                $parts = array_filter(array_map(function ($v) { return trim($v); }, explode(',', $productSuperCategoryIds)));
                $productSuperCategoryIds = array_map(function ($v) { return is_numeric($v) ? (int)$v : $v; }, $parts);
            }
        }
        if (!is_array($productSuperCategoryIds)) { $productSuperCategoryIds = []; }
        $categories = Category::where(['parent_id' => 0])->get();
        $mainCategories = MainCategory::active()->get();
        return view('admin-views.product.edit', compact('product', 'product_category', 'categories', 'mainCategories', 'productSuperCategoryIds', 'rootCategoryId', 'subCategoryId', 'subSubCategoryId'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle super categories
        if ($request->has('super_category_ids') && is_array($request->super_category_ids)) {
            $p->super_category_ids = $request->super_category_ids;
        } else {
