...
class ProductController extends Controller
{
    public function get_latest_products(Request $request)
    {
        ...
        $products = ProductLogic::get_latest_products(limit:$request->query('limit'), offset:$request->query('offset'),restaurant_id: $request->query('restaurant_id'),category_id: $request->query('category_id'), type:$type);
    ...
    public function get_searched_products(Request $request)
    {
        ...
        $products = Food::active()
            ->currentlyAvailable($request->meal ?? null)
            ->type($type)
    ...
    public function get_popular_products(Request $request)
{
    ...
    // Fetch delivery time for each product
    foreach ($products['products'] as &$product) {
        // Assuming each product has a restaurant_id
        $restaurant_id = $product['restaurant_id']; // Adjust according to your product structure

        // Fetch delivery time from the restaurant model or service
        $delivery_time = Restaurant::find($restaurant_id)->delivery_time; // or any method you have to get delivery time

        // Include delivery_time in the product data
...
public function get_most_reviewed_products(Request $request)
{
    ...
    $products = ProductLogic::most_reviewed_products(
        zone_id: $zone_id,
        limit: $request['limit'],
    ...
    $formattedProducts = Helpers::product_data_formatting(
    ...
    // Add delivery_time to each product
    foreach ($formattedProducts as &$product) {
        // Assuming each product has a 'restaurant_id' field to fetch delivery_time
        $restaurant = Restaurant::find($product['restaurant_id']);
        $product['delivery_time'] = $restaurant ? $restaurant->delivery_time : null;
    }
    ...
    {
        try {
            $products = Helpers::product_data_formatting(data:Food::active()->currentlyAvailable($request->meal ?? null)->with(['rating'])->where(['set_menu' => 1, 'status' => 1])->get(),multi_data: true, trans:false, local:app()->getLocale());
            return response()->json($products, 200);
        } catch (\Exception $e) {
            info($e->getMessage());
            return response()->json([
    ...
    public function get_product_reviews($food_id)
{
    $reviews = Review::with(['customer', 'food.restaurant'])
    ...
    foreach ($reviews as $item) {
        ...
        if ($item?->food) {
            ...
            if (count($item->food->translations) > 0) {
                $translate = array_column($item->food->translations->toArray(), 'value', 'key');
                $item['food_name'] = $translate['name'] ?? $item['food_name'];
            }

            if ($item->food->restaurant) {
                $item['delivery_time'] = $item->food->restaurant->delivery_time;
            }
        }
