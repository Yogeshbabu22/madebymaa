...
use App\Models\OrderAddloggsDelivery;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Log;
use App\Services\AdloggsService;
...
class OrderController extends Controller
{
    ...
    public function __construct(AdloggsService $adloggs)
    {
        $this->adloggs = $adloggs;
    }
...
//     public function get_food_available_times($food_id)
// {
//     // Find the food item by its ID
//     $food = Food::find($food_id);

//     // If food not found, return an error response
//     if (!$food) {
//         return response()->json(['error' => 'Food item not found'], 404);
//     }

//     // Decode the available times JSON
//     $available_times = json_decode($food->available_times, true);

//     // If the available_times JSON is invalid or empty
//     if (json_last_error() !== JSON_ERROR_NONE || empty($available_times)) {
//         return response()->json(['error' => 'No available times found for this food item'], 404);
//     }

//     // Prepare the response data for food's available times
//     $preferences = [];

//     // Loop through each day and its slots
//     foreach ($available_times as $day => $times) {
//         // Check if the day has available slots
//         if (!empty($times['slots'])) {
//             $preferences[] = [
//                 'day' => ucfirst($day),  // Capitalize day name
//                 'slots' => $times['slots']
//             ];
//         }
//     }

//     // If no slots found for any day
//     if (empty($preferences)) {
//         return response()->json(['error' => 'No available time slots found'], 404);
//     }

//     // Return the available time preferences for the food
//     return response()->json([
//         'success' => 'Available times fetched successfully',
//         'data' => $preferences
//     ]);
// }
public function getAvailableTimes($food_id)
{
    // Query the food_schedules table to get all schedules for the given food ID
    $schedules = FoodSchedule::where('food_id', $food_id)->get();

    // If no schedules found, return an error response
    if ($schedules->isEmpty()) {
        return response()->json(['error' => 'No available times found for this food item']);
    }

    // Initialize an array to organize the available times by day
    $availableTimes = [
        'sunday' => ['slots' => []],
        'monday' => ['slots' => []],
        'tuesday' => ['slots' => []],
    ...
    // Loop through each schedule and organize it into the appropriate day
    foreach ($schedules as $schedule) {
        // Format each slot with start and end times
        $availableTimes[strtolower($schedule->day)]['slots'][] = [
            'start' => $schedule->available_time_start,
            'end' => $schedule->available_time_end,
        ];
    }
    ...
    //     // Get main_category_id from cart items (for category-wise schedule check)
    //     $mainCategoryId = null;
    //     $user_id = $request->user ? $request->user->id : $request['guest_id'];
    //     $is_guest = $request->user ? 0 : 1;
    //     $cartQuery = Cart::where('user_id', $user_id)->where('is_guest', $is_guest);
    //     if (isset($request->is_buy_now) && $request->is_buy_now == 1 && $request->cart_id) {
    //         $cartQuery = $cartQuery->where('id', $request->cart_id);
    //     }
    //     $cartItems = $cartQuery->get();

    //     // Get main_category_id from cart items by checking food schedules
    //     foreach ($cartItems as $cartItem) {
    //         if ($cartItem->item_type === 'App\\Models\\Food') {
    //             $food = \App\Models\Food::find($cartItem->item_id);
    //             if ($food) {
    //                 // Check food schedules to get main_category_id
    //                 $foodSchedule = \App\Models\FoodSchedule::where('food_id', $food->id)
    //                     ->where('day', strtolower($schedule_at->format('l')))
    //                     ->first();
    //                 if ($foodSchedule && $foodSchedule->main_category_id) {
    //                     $mainCategoryId = $foodSchedule->main_category_id;
    //                     break;
    //                 }

    //                 // Fallback: Try to get from super_category_ids
    //                 if ($food->super_category_ids) {
    //                     $superCategoryIds = is_array($food->super_category_ids)
    //                         ? $food->super_category_ids
    //                         : explode(',', $food->super_category_ids);
    //                     $superCategoryIds = array_filter($superCategoryIds);
    //                     if (!empty($superCategoryIds)) {
    //                         $superCategory = \App\Models\Category::find($superCategoryIds[0]);
    //                         if ($superCategory && $superCategory->main_category_id) {
    //                             $mainCategoryId = $superCategory->main_category_id;
    //                             break;
    //                         }
    //                     }
    //                 }
    ...
    //     // If main_category_id not found from cart, detect from order time
    //     if (!$mainCategoryId) {
    //         $orderTime = $schedule_at->format('H:i:s');
    //         $mainCategories = \App\Models\MainCategory::active()->get();
    //         foreach ($mainCategories as $mainCat) {
    //             if ($mainCat->start_time && $mainCat->end_time) {
    //                 $startTime = is_string($mainCat->start_time) ? $mainCat->start_time : $mainCat->start_time->format('H:i:s');
    //                 $endTime = is_string($mainCat->end_time) ? $mainCat->end_time : $mainCat->end_time->format('H:i:s');
    ...
    //     // Build restaurant query with main_category_id check
    //     $dayNumberIso = (int)$schedule_at->format('N'); // 1 (Monday) through 7 (Sunday)
    //     $dayConditions = '`restaurant_schedule`.`day` = ' . $dayNumberIso;

    //     $scheduleQuery = '(select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id` and ('.$dayConditions.') and `restaurant_schedule`.`opening_time` <= "'.$schedule_at->format('H:i:s').'" and `restaurant_schedule`.`closing_time` >= "'.$schedule_at->format('H:i:s').'"';

    //     // Add main_category_id check if available
    //     if ($mainCategoryId) {
    //         // Check if specific category is available OR general schedule (NULL) is available
    //         $scheduleQuery .= ' and (`restaurant_schedule`.`main_category_id` = '.$mainCategoryId.' or `restaurant_schedule`.`main_category_id` IS NULL)';
    //     }
    //     $scheduleQuery .= ') > 0';

    //     $restaurant = Restaurant::with(['discount', 'restaurant_sub'])
    //         ->selectRaw('*, IF('.$scheduleQuery.', true, false) as open')
    //         ->where('id', $request->restaurant_id)
    //         ->first();

    //     $orderSuperSet = [];
    //     foreach ($cartItems as $ci) {
    //         if ($ci->item_type === 'App\\Models\\Food') {
    //             $food = Food::find($ci->item_id);
    //             $ids = (array) ($food?->super_category_ids ?? []);
    //             foreach ($ids as $sid) {
    //                 if ($sid !== null && $sid !== '') {
    //                     $orderSuperSet[(string) $sid] = true;
    //                 }
    //             }
    //         }
    //     }
    //     if (count($orderSuperSet) > 1) {
    //         return response()->json([
    //             'errors' => [
    //                 ['code' => 'super_category_mismatch', 'message' => translate('messages.you_cannot_mix_breakfast_lunch_dinner_in_one_order')]
    //             ]
    //         ], 409);
    //     }
    ...
    //     // Removed blocking condition for computed closed; allowing order to proceed.
...
//         // Build schedule query - check for any matching schedule (category-specific or general)
//         // Use OR logic: check for category-specific schedule first, if not found, check for general schedule
//         if ($mainCategoryId) {
//             // If main_category_id is found, check for category-specific schedule OR general schedule (NULL)
//             $scheduleQuery = '(select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id` and ('.$dayConditions.') and `restaurant_schedule`.`opening_time` <= "'.$schedule_at->format('H:i:s').'" and `restaurant_schedule`.`closing_time` >= "'.$schedule_at->format('H:i:s').'" and (`restaurant_schedule`.`main_category_id` = '.$mainCategoryId.' or `restaurant_schedule`.`main_category_id` IS NULL)) > 0';
//         } else {
//             // If no main_category_id, check for any schedule (no category restriction)
//             $scheduleQuery = '(select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id` and ('.$dayConditions.') and `restaurant_schedule`.`opening_time` <= "'.$schedule_at->format('H:i:s').'" and `restaurant_schedule`.`closing_time` >= "'.$schedule_at->format('H:i:s').'") > 0';
//         }

//         // Get restaurant - if schedule check fails, still allow order (open will be false but we won't block)
//         $restaurant = Restaurant::with(['discount', 'restaurant_sub'])
//             ->selectRaw('*, IF('.$scheduleQuery.', true, false) as open')
//             ->where('id', $request->restaurant_id)
//             ->first();
...
//             $superCategories = is_array($food->super_category_ids)
//                 ? $food->super_category_ids
//                 : explode(',', $food->super_category_ids);

//             $superCategories = array_filter($superCategories);

//             // ❗ check: selected main category food me allowed hai?
//             if (!in_array($mainCategoryId, $superCategories)) {
//                 return response()->json([
//                     'errors' => [
//                         [
//                             'code' => 'super_category_mismatch',
//                             'message' => translate('messages.you_cannot_mix_breakfast_lunch_dinner_in_one_order')
//                         ]
//                     ]
//                 ], 409);
//             }
...
//         // Get instant_order setting
//         $instant_order = Helpers::get_business_settings('instant_order', false);
//         $instant_order = $instant_order == 1 || $instant_order === '1';
...
//                     // ====== Third Party Delivery Integration (Adloggs) ======
//                     try {
//                         // Check if third party delivery is enabled and restaurant uses it
//                         $thirdPartyDeliveryEnabled = Helpers::get_business_settings('third_party_delivery_status', false);
//                         $thirdPartyDeliveryConfig = Helpers::get_business_settings('third_party_delivery_config') ?? [];
//                         $restaurantDeliveryPartner = $restaurant->delivery_partner ?? '';

//                         // Force default partner to Adloggs when third party is enabled but not yet set on restaurant
//                         if (($thirdPartyDeliveryEnabled == 1 || $thirdPartyDeliveryEnabled === '1') && empty($restaurantDeliveryPartner) && $restaurant->delivery) {
//                             $restaurant->delivery_partner = 'adloggs';
//                             $restaurant->save();
//                             $restaurantDeliveryPartner = 'adloggs';
//                             Log::info('Third Party Delivery: Auto-assigned delivery partner', [
//                                 'restaurant_id' => $restaurant->id,
//                                 'delivery_partner' => $restaurantDeliveryPartner
//                             ]);
//                         }

//                         // Debug logging
//                         Log::info('Third Party Delivery: Pre-check', [
//                             'order_id' => $order->id,
//                             'order_type' => $request->order_type,
//                             'third_party_delivery_enabled' => $thirdPartyDeliveryEnabled,
//                             'restaurant_delivery_partner' => $restaurantDeliveryPartner,
//                             'restaurant_id' => $restaurant->id,
//                             'conditions_met' => [
//                                 'is_delivery' => $request->order_type == 'delivery',
//                                 'third_party_enabled' => ($thirdPartyDeliveryEnabled == 1 || $thirdPartyDeliveryEnabled === '1'),
//                                 'restaurant_uses_adloggs' => $restaurantDeliveryPartner === 'adloggs'
//                             ]
//                         ]);

//                         if ($request->order_type == 'delivery'
//                             && ($thirdPartyDeliveryEnabled == 1 || $thirdPartyDeliveryEnabled === '1')
//                             && $restaurantDeliveryPartner === 'adloggs') {

//                             $adloggs = new AdloggsService();

//                             // Get default values from business settings
//                             $defaultCountry = $thirdPartyDeliveryConfig['default_country'] ?? 'India';
//                             $orderCategory = $thirdPartyDeliveryConfig['order_category'] ?? 'Food and Beverage';
//                             $defaultWeight = $thirdPartyDeliveryConfig['default_weight_kg'] ?? '1.00';
//                             $utcOffset = $thirdPartyDeliveryConfig['utc_offset'] ?? 330;

//                             // Check if this is a scheduled order
//                             $isScheduledOrder = $request->schedule_at && $order->scheduled == 1;
//                             $pickupDateTime = $isScheduledOrder ? $schedule_at : now();

//                             // Prepare pickup & drop data
//                             $pickup = [
...
//                             // 1) Check availability and estimated price
//                             Log::info('Third Party Delivery: Checking availability', [
//                                 'order_id' => $order->id,
//                                 'restaurant_id' => $restaurant->id,
//                                 'is_scheduled' => $isScheduledOrder,
//                                 'schedule_at' => $isScheduledOrder ? $schedule_at->format('Y-m-d H:i:s') : null,
//                                 'pickup' => $pickup,
//                                 'drop' => $drop
//                             ]);

//                             $availResp = $adloggs->checkAvailability($pickup, $drop, $request->payment_method === 'cash_on_delivery' ? 'COD' : 'Online');

//                             Log::info('Third Party Delivery: Availability response', [
//                                 'order_id' => $order->id,
//                                 'response' => $availResp
//                             ]);

//                             if (!($availResp['status'] ?? false) || !($availResp['data']['service_available'] ?? false)) {
//                                 // log and rollback safe: let client know delivery not possible
//                                 Log::warning('Third Party Delivery: Service not available', [
//                                     'order_id' => $order->id,
//                                     'response' => $availResp
//                                 ]);
//                                 DB::rollBack();
//                                 return response()->json([
//                                     'errors' => [
//                                         ['code' => 'delivery_unavailable', 'message' => $availResp['message'] ?? translate('messages.third_party_delivery_not_available')]
//                                     ]
//                                 ], 422);
//                             }

//                             // Update order delivery charge from Adloggs estimate
//                             $order->delivery_partner = 'adloggs';
//                             $order->delivery_fee = $availResp['data']['estimated_price'] ?? ($order->delivery_charge ?? 0);
//                             $order->delivery_distance = $availResp['data']['distance'] ?? $order->delivery_distance;
//                             $order->save();

//                             // 2) Create Adloggs order
//                             $adloggsConfig = Helpers::get_business_settings('adloggs_config') ?? [];
//                             $partnerMerchantId = $adloggsConfig['partner_merchant_id'] ?? env('ADLOGGS_PARTNER_ID', 5421);

//                             $createPayload = [
//                                 "partner_order_id" => (string)$order->id,
//                                 "partner_merchant_id" => $partnerMerchantId,
//                                 "pickup_contact_name" => $restaurant->name ?? translate('messages.restaurant'),
//                                 "pickup_contact_no" => $restaurant->phone ?? '',
//                                 "pickup_address" => $restaurant->address ?? '',
//                                 "pickup_address_details" => [
//                                     "city_name" => $restaurant->city ?? '',
//                                     "state_name" => $restaurant->state ?? '',
//                                     "country_name" => $restaurant->country ?? $defaultCountry,
//                                     "pincode" => $restaurant->pincode ?? ''
//                                 ],
//                                 "pickup_date_time" => $pickupDateTime->format('Y-m-d H:i'),
//                                 "pickup_lat" => (float) ($restaurant->latitude ?? 0),
//                                 "pickup_long" => (float) ($restaurant->longitude ?? 0),
//                                 "delivery_contact_name" => ($address['contact_person_name'] ?? '') ?: ($request->user->f_name ?? translate('messages.customer')),
//                                 "delivery_contact_no" => ($address['contact_person_number'] ?? '') ?: ($request->user->phone ?? ''),
//                                 "delivery_address" => $address['address'] ?? '',
//                                 "delivery_address_details" => [
//                                     "city_name" => $address['city_name'] ?? '',
//                                     "state_name" => $address['state_name'] ?? '',
//                                     "country_name" => $address['country_name'] ?? $defaultCountry,
//                                     "pincode" => $address['pincode'] ?? ''
//                                 ],
//                                 "delivery_lat" => (float) ($request->latitude ?? 0),
//                                 "delivery_long" => (float) ($request->longitude ?? 0),
//                                 "order_category" => $orderCategory,
//                                 "order_total_price" => (string)$order->order_amount,
//                                 "order_total_weight_in_kg" => $defaultWeight,
//                                 "items" => [
//                                     [
//                                         "name" => "Order #{$order->id}",
//                                         "quantity" => "1",
//                                         "price" => (string)$order->order_amount
//                                     ]
//                                 ],
//                                 "utc_offset" => $utcOffset,
//                                 "payment_type" => $request->payment_method === 'cash_on_delivery' ? "COD" : "Online",
//                                 "collectible_amount" => $request->payment_method === 'cash_on_delivery' ? (float)$order->order_amount : 0,
//                             ];

//                             Log::info('Third Party Delivery: Creating order on Adloggs', [
//                                 'order_id' => $order->id,
//                                 'is_scheduled' => $isScheduledOrder,
//                                 'pickup_date_time' => $pickupDateTime->format('Y-m-d H:i:s'),
//                                 'payload' => $createPayload
//                             ]);

//                             $adResp = $adloggs->createOrder($createPayload);

//                             if ($adResp && ($adResp['status'] ?? false) === true && isset($adResp['data'])) {
//                                 $order->adloggs_order_uuid = $adResp['data']['order_uuid'] ?? null;
//                                 $order->delivery_tracking_url = $adResp['data']['trackUrl'] ?? null;
//                                 $order->delivery_fee = $adResp['data']['fee'] ?? $order->delivery_fee;
//                                 $order->delivery_distance = $adResp['data']['distance'] ?? $order->delivery_distance;
//                                 $order->delivery_status = $isScheduledOrder ? 'scheduled' : 'pending';
//                                 $order->save();

//                                 Log::info('Third Party Delivery: Order created successfully on Adloggs', [
//                                     'order_id' => $order->id,
//                                     'is_scheduled' => $isScheduledOrder,
//                                     'adloggs_order_uuid' => $order->adloggs_order_uuid,
//                                     'tracking_url' => $order->delivery_tracking_url,
//                                     'delivery_status' => $order->delivery_status,
//                                     'pickup_date_time' => $pickupDateTime->format('Y-m-d H:i:s')
//                                 ]);
//                             } else {
//                                 // failed to create order on Adloggs — log and continue (you can rollback if you want)
//                                 Log::error('Third Party Delivery: Order creation failed on Adloggs', [
//                                     'order_id' => $order->id,
//                                     'response' => $adResp,
//                                     'payload' => $createPayload
//                                 ]);
//                                 // optionally rollback as service is required:
//                                 // DB::rollBack(); return response()->json([...], 422);
//                             }
//                         }
//                     } catch (\Exception $e) {
//                         // Log error but don't block order placement
//                         Log::error('Third Party Delivery: Exception occurred', [
//                             'order_id' => $order->id ?? null,
//                             'error' => $e->getMessage(),
//                             'trace' => $e->getTraceAsString()
//                         ]);
//                         // Continue with order placement even if third party delivery fails
//                     }
...
//             $order_verification_mail_status = Helpers::get_mail_status('order_verification_mail_status_user');
//      try {
//     $result = Helpers::send_order_notification($order);
//     Log::info('Notification sent result:', ['result' => $result, 'order_id' => $order->id]);
// } catch (\Exception $e) {
  ...
  public function place_order(Request $request)
    {
        ...
        // Detect main_category_id from order time FIRST
        if (!$mainCategoryId) {
            $orderTime = $schedule_at->format('H:i:s');
            $mainCategories = \App\Models\MainCategory::active()->get();
            foreach ($mainCategories as $mainCat) {
        ...
        // Fallback: Get main_category_id from cart items if not determined by time
        if (!$mainCategoryId) {
            foreach ($cartItems as $cartItem) {
                if ($cartItem->item_type === 'App\\Models\\Food') {
                    $food = \App\Models\Food::find($cartItem->item_id);
        ...
        // Log schedule check result for debugging
        Log::info('Restaurant Schedule Check', [
            'restaurant_id' => $request->restaurant_id,
            'schedule_at' => $schedule_at->format('Y-m-d H:i:s'),
            'main_category_id' => $mainCategoryId,
            'restaurant_open' => $restaurant->open ?? false,
        ...
        // Removed blocking condition for computed closed; allowing order to proceed.
        // Commented out to allow orders even if instant_order is disabled
        // if(!$instant_order && $restaurant->restaurant_config?->instant_order != 1 && !$request->schedule_at && !$request->subscription_order){
        //     return response()->json([
        //         'errors' => [
        //             ['code' => 'instant_order', 'message' => translate('messages.instant_order_is_not_available_for_now!')]
        //         ]
    ...
    public function get_order_subscription_list(Request $request)
    {
        ...
        $orders = array_map(function ($data) {
            ...
            $data['is_reviewed'] =   $data['details_count'] >  Review::whereOrderId($data->id)->count() ? False :True ;
            $data['is_dm_reviewed'] =  $data['delivery_man'] ? DMReview::whereOrderId($data->id)->exists()  : True ;
   $addloggDetail = OrderAddloggsDelivery::where('order_id',$data['id'])->first();
            // dd($addloggDetail);
            $data->addloggs_details = $addloggDetail ;
            return $data;
        }, $paginator->items());
        $data = [
            'total_size' => $paginator->total(),
    ...
    public function get_order_details(Request $request)
{
    ...
    $user_id = $request->user ? $request->user->id : $request['guest_id'];
    $order = Order::with('details', 'offline_payments', 'subscription.schedules')
    ...
    $addloggDetail = OrderAddloggsDelivery::where('order_id', $order->id)->first();
    ...
    if ($order && $order->details->isNotEmpty()) {
        ...
        foreach ($order->details as $item) {
            ...
            $item['addloggs_details'] = !empty($addloggDetail) ? $addloggDetail : null;
