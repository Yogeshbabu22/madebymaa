...
class CartController extends Controller
{
    public function get_carts(Request $request)
    {
        ...
        $startTime = \Carbon\Carbon::createFromFormat('h:i A', $startTime)->format('H:i:s');
        $endTime = \Carbon\Carbon::createFromFormat('h:i A', $endTime)->format('H:i:s');
        }else{
                $startTime = null;
                $endTime = null;
            }
        ...
        ->map(function ($data) use ($startTime,$endTime,$time,$day) {
            ...
            $data->add_on_qtys, false, app()->getLocale());
                ...
                $isAvailable = DB::table('food_schedules')->where('food_id',$data->item->id)
                ->where('day',$day)
                ->where(function ($query) use ($startTime,$endTime){
                    $query->where('available_time_start','<=',$endTime)
                    ->where('available_time_end','>=',$startTime);
                })
                ->exists();
                $data->available_food_time = $isAvailable ? true : false;

            }else{
                $data->available_food_time = null;
            }
			return $data;
		});
    ...
    public function add_to_cart(Request $request)
    {
        ...
        // Enforce single super category (e.g., Breakfast/Lunch/Dinner) per cart
        if ($request->model === 'Food') {
            $newItemSuperIds = (array) ($item?->super_category_ids ?? []);
            // Normalize to strings for consistent comparison
            $newItemSuperIds = array_filter(array_map(fn($v) => (string) $v, $newItemSuperIds));

            $existingFood = Cart::where('user_id', $user_id)
                ->where('is_guest', $is_guest)
                ->where('item_type', 'App\\Models\\Food')
                ->first();

            if ($existingFood) {
                $existingFoodModel = Food::find($existingFood->item_id);
                $existingSuperIds = (array) ($existingFoodModel?->super_category_ids ?? []);
                $existingSuperIds = array_filter(array_map(fn($v) => (string) $v, $existingSuperIds));

                // If both have super ids but no overlap, block adding
                if (!empty($newItemSuperIds) && !empty($existingSuperIds)) {
                    $overlap = array_intersect($newItemSuperIds, $existingSuperIds);
                    if (count($overlap) === 0) {
                        return response()->json([
                            'errors' => [
                                ['code' => 'super_category_mismatch', 'message' => translate('messages.you_cannot_mix_breakfast_lunch_dinner_in_one_cart')]
                            ]
                        ], 409);
                    }
                }
