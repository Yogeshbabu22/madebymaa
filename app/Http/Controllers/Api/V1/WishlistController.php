...
//     // Get zoneId header and ensure it’s an array
...
//     // Retrieve wishlist items with restaurants and food data, applying conditions
//     $wishlists = Wishlist::where('user_id', $request->user()?->id)
//         ->with([
//             'food' => function($q) use ($zone_id) {
//                 $q->whereHas('restaurant', function($q) use ($zone_id) {
//                     $q->whereIn('zone_id', $zone_id);
//                 });
//             },
//             'restaurant' => function($q) use ($zone_id, $longitude, $latitude) {
//                 // Calculate distance and restaurant open status
//                 $q->select('*')
//                   ->selectRaw('(IF(((select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id` and `restaurant_schedule`.`day` = ? and `restaurant_schedule`.`opening_time` < ? and `restaurant_schedule`.`closing_time` > ?) > 0), true, false)) as open', 
//                               [now()->dayOfWeek, now()->format('H:i:s'), now()->format('H:i:s')])
//                   ->whereIn('zone_id', $zone_id)
//                   ->withOpen($longitude, $latitude) // Apply the 'withOpen' condition
//                   ->withCount('foods');
//             }
//         ])
//         ->get();

//     // Format wishlist data using Helpers
//     $wishlists = Helpers::wishlist_data_formatting(data: $wishlists, multi_data: true);
