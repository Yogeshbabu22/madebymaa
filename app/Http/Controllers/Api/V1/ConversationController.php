...
public function conversations(Request $request)
{
    ...
    // Process each conversation for custom logic
    $conversations->getCollection()->transform(function ($conversation) {
        // Format names for sender and receiver
        $conversation->sender->f_name = $this->formatName($conversation->sender->f_name);
        $conversation->receiver->f_name = $this->formatName($conversation->receiver->f_name);

        // Add owner_name if receiver is a vendor
        if ($conversation->receiver && isset($conversation->receiver->vendor_id)) {
            $vendorData = DB::table('vendors')
                ->join('restaurants', 'vendors.id', '=', 'restaurants.vendor_id')
                ->select('vendors.f_name', 'vendors.l_name')
                ->where('vendors.id', $conversation->receiver->vendor_id)
                ->first();

            if ($vendorData) {
                $conversation->receiver->owner_name = $vendorData->f_name . ' ' . $vendorData->l_name;
            }
        }
...
/**
 * Format name to ensure it is a single name.
 */
private function formatName($name)
{
    if (is_string($name) && $this->isJson($name)) {
        $nameArray = json_decode($name, true);
        return is_array($nameArray) ? $nameArray[0] : $name;
    }
    return $name;
}
...
// public function conversations(Request $request)
// {
//     // Get pagination parameters from request or set defaults
//     $limit = $request->input['limit', 10];
//     $offset = $request->input['offset', 1];

//     // Retrieve or create the sender's information
...
//     $conversationsQuery = Conversation::with(['sender', 'receiver', 'last_message'])
//         ->where(function ($q) use ($sender) {
...
//     if ($request->has('type')) {
//         $conversationsQuery->where(function ($q) use ($request) {
...
//     // Paginate the conversations, sorted by last message time
//     $conversations = $conversationsQuery->orderBy('last_message_time', 'DESC')
//                                         ->paginate($limit, ['*'], 'page', $offset);

//     // Fetch vendor data and append 'owner_name' to the receiver
//     $conversations->getCollection()->transform(function ($conversation) {
//         if ($conversation->receiver) {
//             // Vendor data logic, ensuring receiver and vendor_id exist
//             if (isset($conversation->receiver->vendor_id)) {
//                 // Use DB facade to fetch the vendor and restaurant data
//                 $vendorData = DB::table('vendors')
//                     ->join('restaurants', 'vendors.id', '=', 'restaurants.vendor_id')
//                     ->select('vendors.f_name', 'vendors.l_name')
//                     ->where('vendors.id', $conversation->receiver->vendor_id)
...
//                 if ($vendorData) {
//                     // Concatenate the vendor's f_name and l_name to create owner_name
//                     $ownerName = $vendorData->f_name . ' ' . $vendorData->l_name;
//                     $conversation->receiver->owner_name = $ownerName;
//                 }
//             }

//             // Handle the case where f_name is a list (string representation of an array)
//             if (is_string($conversation->receiver->f_name)) {
//                 $decodedNames = json_decode($conversation->receiver->f_name, true);
//                 if (is_array($decodedNames) && count($decodedNames) > 0) {
//                     $conversation->receiver->f_name = $decodedNames[0];  // Get only the first name
//                 }
//             }
//         }

//         // Handling the 'file' field to make sure it's a valid string, not a string representation of an array
//         if (isset($conversation->last_message->file)) {
//             $file = $conversation->last_message->file;
            
//             // If the 'file' is a string representing an array, decode it to get the file(s)
//             if (is_string($file)) {
//                 $decodedFile = json_decode($file, true);
                
//                 // If it's an array, get the first file or join files as a comma-separated string
//                 if (is_array($decodedFile) && count($decodedFile) > 0) {
//                     $conversation->last_message->file = $decodedFile[0];  // If you need only the first file
//                 } else {
//                     // Otherwise, return the file as a string
//                     $conversation->last_message->file = $file;  // Leave as is
//                 }
//             }
//         }
...
//     // Return response with the formatted data
