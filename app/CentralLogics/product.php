...
class ProductLogic
{
    ...
    public static function get_latest_products($limit, $offset, $restaurant_id, $category_id, $type='all')
    {
        ...
        $paginator = $paginator->where('restaurant_id', $restaurant_id)
                     ->whereHas('vendorFoodOrders', function ($query) {
                        $query->where('pre_order', true)->orWhere('instant_order', true);
                     })
        ->latest()
    ...
    public static function popular_products($zone_id, $limit = null, $offset = null, $type = 'all')
{
    $query = Food::active()
        ...
        ->whereHas('vendorFoodOrders', function($query) {
            // Ensure that pre_order (schedule_order) or instant_order is true
            $query->where(function($query) {
                $query->where('pre_order', true)
                      ->orWhere('instant_order', true);
            });
        })
        ->has('reviews')
        ->type($type)
