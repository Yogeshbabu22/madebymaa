    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en')
    {
        ...
        if ($multi_data == true) {
            ...
            foreach ($data as $item) {
                ...
                $vendorfoodorder = VendorFoodOrder::Where('food_id',$item->id)->first();
                //if($vendorfoodorder){
                     $item->schedule_orders = !empty($vendorfoodorder->pre_order) ? $vendorfoodorder->pre_order : null;
                $item->instant_order = !empty($vendorfoodorder->instant_order) ? $vendorfoodorder->instant_order : null;
               // }
...
// Check if decoding was successful and if `$addOnIds` is an array

if (!is_array($addOnIds)) {
    // If it's not an array, we either set it as an empty array or try to handle a single ID
    $addOnIds = is_numeric($item['add_ons']) ? [(int)$item['add_ons']] : [];
}
                $item['add_ons'] = self::addon_data_formatting(AddOn::whereIn('id', $addOnIds)->active()->get(), true, $trans, $local);
                $item['tags'] = $item->tags;
    ...
    // Safely handle category_ids
    if (is_string($data?->category_ids)) {
        foreach (json_decode($data->category_ids) as $value) {
            $categories[] = ['id' => (string)$value->id, 'position' => $value->position];
        }
    } elseif (is_array($data?->category_ids)) {
        foreach ($data->category_ids as $value) {
            $categories[] = ['id' => (string)($value['id'] ?? ''), 'position' => $value['position'] ?? null];
        }
    }

    $data['category_ids'] = $categories;

    // Safely handle add_ons
    if (is_string($data['add_ons'])) {

        $data['add_ons'] = self::addon_data_formatting(AddOn::whereIn('id', json_decode($data['add_ons']))->active()->get(), true, $trans, $local);
    } else {
        $data['add_ons'] = []; // Default to an empty array if add_ons is not a valid JSON string
    }
    ...
    // Check if variations is a valid JSON string
    if (is_string($data['variations'])) {
        $data['variations'] = json_decode($data['variations'], true);
    } else {
        $data['variations'] = []; // Default to an empty array if variations is not valid
    }
    ...
    // Handle restaurant ratings safely
    try {
        $reviewsInfo = $data->rating()->first();
    } catch (\Exception $e) {
        $reviewsInfo = null; // Set to null if an exception occurs
    }
    ...
    if (self::getDeliveryFee($data->restaurant) == 'free_delivery') {
        $data['free_delivery'] = 1; // Set to 1 if free delivery
    }

    // Handle delivery times
    $delivery_time = explode('-', $data->restaurant->delivery_time);
    $data['min_delivery_time'] = (int) ($delivery_time[0] ?? 0);
    $data['max_delivery_time'] = (int) ($delivery_time[1] ?? 0);
    ...
    unset($data['rating']);
     ...
     public static function product_data_formattings($data, $multi_data = false, $trans = false, $local = 'en')
    {
        $storage = [];
        if ($multi_data == true) {
            foreach ($data as $item) {
    ...
    public static function product_data_formatting_translate($data, $multi_data = false, $trans = false, $local = 'en')
    {
        ...
        if ($multi_data == true) {
            ...
            $addOnIds = json_decode($data['add_ons'], true);

// Check if decoding was successful and if `$addOnIds` is an array
if (!is_array($addOnIds)) {
    // If it's not an array, we either set it as an empty array or try to handle a single ID
    $addOnIds = is_numeric($data['add_ons']) ? [(int)$data['add_ons']] : [];
}
            $data['add_ons'] = self::addon_data_formatting(AddOn::whereIn('id', $addOnIds)->active()->get(), true, $trans, $local);
    ...
    public static function restaurant_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item->load('cuisine');
                // Load vendor to compute owner_name
                $item->load('vendor');
            ...
            $data->load(['cuisine','vendor']);
            $data['cuisine'] = $data->cuisine;

            // Add owner_name for single restaurant payload
            $ownerFirstName = $data->vendor->f_name ?? '';
            $ownerLastName = $data->vendor->l_name ?? '';
            $ownerFullName = trim($ownerFirstName.' '.$ownerLastName);
            $data['owner_name'] = $ownerFullName !== '' ? $ownerFullName : null;
...
//         foreach ($data as $item) {
//             $item['foods'] = $item->foods()->active()->take(5)->get(['id', 'image', 'name']);
//             $item->load(['cuisine', 'vendor']);  // Load the vendor relationship
//             $restaurant_id = (string)$item->id;
...
//             // Include vendor_name from the vendor relationship
//             $item['vendor_name'] = $item->vendor->vendor_name ?? null;

//             // Delivery and status logic
//             if ($item->restaurant_model == 'subscription' && isset($item->restaurant_sub)) {
//                 $item['self_delivery_system'] = (int)$item->restaurant_sub->self_delivery;
//             }
...
//             // Additional restaurant configurations
//             $item['customer_order_date'] = (int)$item?->restaurant_config?->customer_order_date;
//             $item['customer_date_order_sratus'] = (bool)$item?->restaurant_config?->customer_date_order_sratus;
//             $item['instant_order'] = (bool)$item?->restaurant_config?->instant_order;
//             $item['halal_tag_status'] = (bool)$item?->restaurant_config?->halal_tag_status;
...
//     } else {
//         // Single data case
//         $data->load(['cuisine', 'vendor']);  // Load the vendor relationship
...
//         // Include vendor_name from the vendor relationship
//         $data['vendor_name'] = $data->vendor->vendor_name ?? null;
...
//            $distance = data_get($distance,'rows.0.elements.0.distance.text',' ');
    ...
    public static function send_push_notif_to_device($fcm_token, $data, $web_push_link = null)
    {
        ...
        if (!$key) {
            info('FCM Device Send: missing push_notification_key');
            return false;
        }
        ...
        if ($web_push_link) {
            $payload['notification']['click_action'] = (string) $web_push_link;
        }
        ...
        try {
            $maskedToken = is_string($fcm_token) ? (substr($fcm_token, 0, 8) . '...' . substr($fcm_token, -6)) : null;
            info('FCM Device Send', [
                'http_code' => $httpCode,
    ...
    public static function send_push_notif_to_topic($data, $topic, $type, $web_push_link = null)
    {
        ...
        $key = BusinessSetting::where(['key' => 'push_notification_key'])->first()?->value;
        if (!$key) {
            info('FCM Topic Send: missing push_notification_key');
            return false;
        }
        ...
        if (isset($data['order_id'])) {
            $payload['data']['order_id'] = $data['order_id'];
            $payload['data']['order_type'] = $order_type;
            $payload['notification']['order_id'] = $data['order_id'];
            $payload['notification']['title_loc_key'] = $data['order_id'];
        }
        if ($web_push_link) {
            $payload['notification']['click_action'] = (string) $web_push_link;
        }
    ...
    public static function send_push_notif_for_demo_reset($data, $topic, $type,)
    {
        $key = BusinessSetting::where(['key' => 'push_notification_key'])->first()?->value;
        if (!$key) {
            info('FCM Demo Reset Send: missing push_notification_key');
            return false;
        }
...
//     {
//         $jsonPath = storage_path('app/public/server.json');
//         \Log::info('FCM JSON Path: ' . $jsonPath);
...
//     /**
//      * Send push notification to a single device (mobile + web)
//      */
//     public static function send_push_notif_to_device($fcm_token, $data, $web_push_link = null)
//     {
...
//             $url = "https://fcm.googleapis.com/v1/projects/madebymaa-633ee/messages:send";
...
//     /**
//      * Send push notification to a topic (mobile + web)
//      */
//     public static function send_push_notif_to_topic($data, $topic, $type, $web_push_link = null)
//     {
//         $accessToken = self::getFcmAccessToken();
...
//     /**
//      * Send demo reset notifications to topic (mobile + web)
//      */
//     public static function send_push_notif_for_demo_reset($data, $topic, $type)
//     {
//         return self::send_push_notif_to_topic($data, $topic, $type);
//     }
