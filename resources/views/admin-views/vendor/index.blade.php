    ...
    <div class="content container-fluid initial-57">
        ...
        <form action="{{ route('admin.restaurant.store') }}" method="post" enctype="multipart/form-data"
            ...
            <div class="row g-2">
                ...
                @php
                    $additional_data = isset($page_data) && is_array($page_data) && isset($page_data['data']) && is_array($page_data['data']) ? $page_data['data'] : [];
                @endphp
                        ...
                        <div class="card-body pb-0">
                            <div class="row">
                                @foreach ($additional_data as $key=>$item)
                                    @if(isset($item) && isset($item['field_type']))
                                    @if (!in_array($item['field_type'], ['file' , 'check_box']) )
                                        ...
                                        @elseif ($item['field_type'] == 'file' )
                                            ...
                                            @php
                                                ...
                                                $pdf = '';
                                                $docs = '';
                                                if(isset($item['media_data']) && data_get($item['media_data'],'image',null)){
                                                    $image = '.jpg, .jpeg, .png,';
                                                }
                                                if(isset($item['media_data']) && data_get($item['media_data'],'pdf',null)){
                                                    $pdf = ' .pdf,';
                                                }
                                                if(isset($item['media_data']) && data_get($item['media_data'],'docs',null)){
                                                    $docs = ' .doc, .docs, .docx';
                                                }
...
@push('script_2')
    ...
    <script>
        ...
        function updateAvailabilityData() {
            ...
            console.log('Availability data updated:', availabilityJson);
        }
        ...
        // Update availability data before form submission
        ...
        $(function() {
            $("#coba").spartanMultiImagePicker({
                ...
                @php
                    ...
                    $default_location = $default_location && $default_location->value ? json_decode($default_location->value, true) : 0;
                @endphp
                let myLatlng = {
