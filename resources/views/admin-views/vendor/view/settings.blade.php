    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
               ...
               @php($thirdPartyDeliveryEnabled = \App\Models\BusinessSetting::where('key', 'third_party_delivery_status')->first()?->value ?? 0)
                    @if($thirdPartyDeliveryEnabled == 1 && $restaurant->delivery)
                    @php(
                        $selectedDeliveryPartner = ($restaurant->delivery_partner === null || $restaurant->delivery_partner === '')
                            ? 'adloggs'
                            : $restaurant->delivery_partner
                    )
                    <div class="col-xl-4 col-md-4 col-sm-6">
                        <div class="form-group">
                            <label class="input-label">{{translate('Third Party Delivery Partner')}}
                                <span data-toggle="tooltip" data-placement="right" data-original-title="{{translate('Select third party delivery service for this restaurant')}}" class="input-label-secondary"><img src="{{dynamicAsset('public/assets/admin/img/info-circle.svg')}}" alt="i"></span>
                            </label>
                            <select name="delivery_partner" class="form-control">
                                <option value="" {{$selectedDeliveryPartner == '' ? 'selected' : ''}}>{{translate('None (Use Own Delivery)')}}</option>
                                <option value="adloggs" {{$selectedDeliveryPartner == 'adloggs' ? 'selected' : ''}}>Adloggs</option>
                            </select>
                        </div>
                    </div>
                    @endif
