...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="mt-4">
            <h4 class="card-title mb-3">
                <i class="tio-delivery mr-1"></i>
                {{translate('Third Party Delivery Settings')}}
            </h4>
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.business-settings.update-order') }}" method="post">
                        @csrf
                        @php($thirdPartyDeliveryStatus = \App\Models\BusinessSetting::where('key', 'third_party_delivery_status')->first())
                        @php($thirdPartyDeliveryStatus = $thirdPartyDeliveryStatus ? $thirdPartyDeliveryStatus->value : 0)
                        @php($thirdPartyDeliveryConfig = \App\Models\BusinessSetting::where('key', 'third_party_delivery_config')->first())
                        @php($thirdPartyDeliveryConfig = $thirdPartyDeliveryConfig ? json_decode($thirdPartyDeliveryConfig->value, true) : [])
                        @php($adloggsConfig = \App\Models\BusinessSetting::where('key', 'adloggs_config')->first())
                        @php($adloggsConfig = $adloggsConfig ? json_decode($adloggsConfig->value, true) : [])
                            ...
                            <div class="col-md-12">
                                <div class="form-group mb-0">
                                    <label class="toggle-switch h--45px toggle-switch-sm d-flex justify-content-between border rounded px-3 py-0 form-control">
                                        <span class="pr-1 d-flex align-items-center switch--label">
                                            <span class="line--limit-1">
                                                {{ translate('Enable Third Party Delivery') }}
                                            </span>
                                            <span class="form-label-secondary text-danger d-flex" data-toggle="tooltip" data-placement="right"
                                                data-original-title="{{ translate('Enable third party delivery services like Adloggs') }}">
                                                <img src="{{ dynamicAsset('/public/assets/admin/img/info-circle.svg') }}" alt="">
                                            </span>
                                        </span>
                                        <input type="checkbox" name="third_party_delivery_status" id="third_party_delivery_status"
                                            value="1" {{ $thirdPartyDeliveryStatus == 1 ? 'checked' : '' }}
                                            class="toggle-switch-input">
                                        <span class="toggle-switch-label">
                                            <span class="toggle-switch-indicator"></span>
                                        </span>
                        ...
                        <div class="row g-3 mt-3">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    <label class="form-label">{{ translate('Default Country') }}</label>
                                    <input type="text" name="default_country" class="form-control"
                                        value="{{ $thirdPartyDeliveryConfig['default_country'] ?? 'India' }}"
                                        placeholder="India">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">{{ translate('Order Category') }}</label>
                                    <input type="text" name="order_category" class="form-control"
                                        value="{{ $thirdPartyDeliveryConfig['order_category'] ?? 'Food and Beverage' }}"
                                        placeholder="Food and Beverage">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">{{ translate('Default Weight (kg)') }}</label>
                                    <input type="text" name="default_weight_kg" class="form-control"
                                        value="{{ $thirdPartyDeliveryConfig['default_weight_kg'] ?? '1.00' }}"
                                        placeholder="1.00">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">{{ translate('UTC Offset') }}</label>
                                    <input type="number" name="utc_offset" class="form-control"
                                        value="{{ $thirdPartyDeliveryConfig['utc_offset'] ?? 330 }}"
                                        placeholder="330">
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <h5 class="card-title mb-3">
                                    <i class="tio-key mr-2"></i>
                                    {{ translate('Adloggs Configuration') }}
                                </h5>
                                <div class="form-group">
                                    <label class="form-label">{{ translate('API Key') }}</label>
                                    <input type="text" name="adloggs_api_key" class="form-control"
                                        value="{{ $adloggsConfig['api_key'] ?? '' }}"
                                        placeholder="Enter Adloggs API Key">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">{{ translate('Base URL') }}</label>
                                    <input type="text" name="adloggs_base_url" class="form-control"
                                        value="{{ $adloggsConfig['base_url'] ?? 'https://dev.adloggs.com' }}"
                                        placeholder="https://dev.adloggs.com">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">{{ translate('Partner Merchant ID') }}</label>
                                    <input type="text" name="adloggs_partner_merchant_id" class="form-control"
                                        value="{{ $adloggsConfig['partner_merchant_id'] ?? '5421' }}"
                                        placeholder="5421">
                                </div>
                            </div>
                        </div>
