...
<script>
    ...
    @php($has_fcm_config = isset($fcm_credentials['projectId']) && !empty($fcm_credentials['projectId']) && isset($fcm_credentials['apiKey']) && !empty($fcm_credentials['apiKey']))
    ...
    function startFCM() {
        if (messaging) {
            ...
            console.log('FCM not initialized - missing configuration');
        }
    }
    @else
    function startFCM() {
        console.log('FCM not initialized - missing Firebase configuration');
    }
    @endif
    ...
    // Only start FCM if configuration is available
    @if($has_fcm_config)
